...
const footerLinks = {
    product: [
        { label: 'Staco Credit', to: '#' },
        { label: 'Staco Payment', to: '#' },
        { label: 'Marketplace', to: '#' },
        { label: 'Staco Services', to: '#' }
    ],
    information: [
        { label: 'Blog', to: '#' },
        { label: 'Support', to: '#' },
        { label: 'Get in touch', to: '#' }
    ],
    resources: [
...
const socialLinks = [
    ...
    { icon: 'i-simple-icons-twitter', to: '#', label: 'Twitter' },
...
<template>
    <footer class="bg-[#1F2334] text-white">
        <UContainer class="py-16">
            <div class="grid md:grid-cols-2 lg:grid-cols-6 gap-12">
                <!-- Brand Section -->
                <div class="lg:col-span-2 space-y-4">
                    <AppLogo class="h-12 w-auto" />
                    <p class="text-gray-400 text-sm leading-relaxed max-w-sm">
                        We denounce with righteous indige nation in and dislike men who are so beguiled and to demo
                        realized by the charms of pleasure
                    </p>
                    <div class="flex gap-4">
                        <UButton v-for="social in socialLinks" :key="social.label" :icon="social.icon" :to="social.to"
                            variant="ghost" color="white" size="sm" :aria-label="social.label" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-3">
                        <li v-for="link in footerLinks.product" :key="link.label">
                            <NuxtLink :to="link.to" class="text-gray-400 hover:text-white transition-colors text-sm">
                                {{ link.label }}
                            </NuxtLink>
                        </li>
                    </ul>
                ...
                <div>
                    ...
                    <ul class="space-y-3">
                        <li v-for="link in footerLinks.information" :key="link.label">
                            <NuxtLink :to="link.to" class="text-gray-400 hover:text-white transition-colors text-sm">
                                {{ link.label }}
                            </NuxtLink>
                        </li>
            ...
            <div
                ...
                <p class="text-gray-400 text-sm">
                    © 2024 Staco. All rights reserved
                </p>
